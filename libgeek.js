function strengthMeter(passwordFieldId, nodes) {
	...
	// Calcul entropie 
	var entropy = Math.ceil(Math.log(strength)/Math.log(2));
    ...
    if (password.length > 0) {
        ...
        $("#entropy").html("Entropie : "+entropy + " bits");
        $("#nodes").val(nodes);
        $("#rates").html(rates);
    } else {
...
/*$(document).ready(function() {
	$("#passwordPwd").keyup(function(event) {
		strengthMeter("passwordPwd",$("#nodes").val());
	});
